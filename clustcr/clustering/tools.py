...
def create_edgelist_vgene(clusters, filename=None, method='HAMMING'):
    '''
    Create tab-separated edgelist of edges with HD = 1, from a set of sequences.    
    '''
    available = ['HAMMING', 'LEVENSHTEIN']
    ...
    tcrs = [
        (
            clusters.iloc[i]["junction_aa"], 
            clusters.iloc[i]["v_call"]
    ...
    if method=='HAMMING':
        ...
        edgelist = set()
        combos = [comb for comb in combinations([tcr[0] for tcr in list(tcrs)], 2)]
        for combo in combos:
            d = lev(combo[0],combo[1])
            if d == 1:
                edgelist.add((combo[0], combo[1]))
